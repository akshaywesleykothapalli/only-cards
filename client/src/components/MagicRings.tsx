...
float ring(vec2 p, float ri, float cut, float t0, float px) {
  float t = uOneTime > 0.5 ? min(uTime + t0, CYCLE) : mod(uTime + t0, CYCLE);
...
export default function MagicRings({
  ...
  propsRef.current = {
    ...
    hoverScale, parallax, clickBurst, oneTimeAnimation,
  };
  ...
  useEffect(() => {
    ...
    const animate = (t: number) => {
      ...
      uniforms.uOneTime.value = p.oneTimeAnimation ? 1.0 : 0.0;
